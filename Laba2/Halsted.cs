...
"await","do", "for", "if","else", "return","while"
        ...
        {
            return (OperatorDictionary.ContainsKey("if") ? OperatorDictionary["if"] : 0) +
                (OperatorDictionary.ContainsKey("case") ? OperatorDictionary["case"] - 1 : 0) +
                (OperatorDictionary.ContainsKey("for") ? OperatorDictionary["for"] : 0) +
                (OperatorDictionary.ContainsKey("while") ? OperatorDictionary["while"] : 0);
        }
        public static double Getcl()
        {
            return (double)GetCL() / OperatorDictionary.Count;
        }
    }
}
